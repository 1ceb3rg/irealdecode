...
const DecodedSongUrlRegex =
  /^ireal(b|book):\/\/[^=]+=[^=]+==[^=]+=[A-Z](b|\#)?-?=\d?\d?=1r34LbKcu7[^=]*=[^=]*=\d\d?\d?=\d\d?\d?/;
...
export function transposeIrealString(
  ...
  return (
    ...
    .split("=")
    .map((section, index) =>
      index !== 5 ? section : Keys[transpose].toString()
    )
    .join("=");
}
...
export function detectPlaylist(link: string): string | undefined {
  const playlist =
    /^(ireal(b|book):\/\/([^=]+=[^=]+==[^=]+=[A-Z](b|\#)?-?=\d?\d?=1r34LbKcu7[^=]*=[^=]*=\d\d?\d?=\d\d?\d?===){1,1400})([^=]*)$/.exec(
